/* @preserve built on: 2025-05-12T10:49:32.177Z */
(function (global, factory) {
    ...
    class DateInput {
        ...
        oninit({ attrs: { value, field } }) {
            ...
            this.valueChange = value.map((newVal) => {
                // only handle value when the main value stream is changed
                if (newVal) {
                    const luxDate = luxon.DateTime.fromISO(String(newVal));
    ...
    class CheckList extends BaseWidget {
        ...
        placeHolder(options, placeholder) {
            ...
            const selected = options.filter(({ value }) => this.selected.has(String(value)));
            if (selected.length > 1) {
                return `${selected.length} Selected`;
            }
            else if (selected.length === 1) {
                return (_a = selected[0].label) !== null && _a !== void 0 ? _a : placeholder;
            }
            else {
                return placeholder;
            }
    ...
    class CheckListGroup extends BaseWidget {
        ...
        static flattenOpts(groups) {
            return lodash.flatMap(groups, ({ groupLabel, groupId, options = [] }) => [
                { groupId, value: groupId, label: groupLabel, header: true },
                ...options.map((option) => (Object.assign({ groupId }, option)))
            ]);
        }
        ...
        selectGroup(groupId) {
            const groupChildren = this.opts.filter((option) => option.groupId === groupId && option.header !== true);
            const selectedChildren = groupChildren.filter((option) => this.selected.has(String(option.value)));
            if (groupChildren.length === selectedChildren.length) {
                this.selected.add(groupId);
            }
        }
        toggleSelection(groupOption, value) {
            ...
            value(this.opts.filter(({ value, header }) => header !== true && this.selected.has(String(value)))
                .map(({ value }) => value)
                .join(","));
            this.changeInput(value);
        }
        ...
        placeHolder(placeholder) {
            ...
            const selected = this.opts.filter(({ value, header }) => header !== true && this.selected.has(String(value)));
            if (selected.length > 1) {
                return `${selected.length} Selected`;
            }
            else if (selected.length === 1) {
