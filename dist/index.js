/* @preserve built on: 2022-09-07T08:20:28.909Z */
...
class CheckLabel {
    view({ attrs: { value, doubleLabel, options, left } }) {
        ...
        lodash.matches({ value: (!doubleLabel ? value : !left) || false }));
...
class CurrencyInput {
    view({ attrs }) {
        const { field, value, xform = value } = attrs;
        const { label, id, name = id, title = label, placeholder, max, maxlength, min, minlength, step, required, readonly, disabled, autofocus, autocomplete, tabindex, pattern, inputmode, spellcheck, uiClass = {}, config, options } = field;
...
function formatCurrency(unitTotal, negativeStyle) {
...
class PercentageInput {
    view({ attrs }) {
        const { field, value, xform = value } = attrs;
        const { label, id, name = id, title = label, placeholder, max, maxlength, min, minlength, step, required, readonly, disabled, autofocus, autocomplete, tabindex, pattern, inputmode, spellcheck, instant, uiClass = {}, config } = field;
        const badgePosition = getConfig("badgePosition", config);
        return m(LayoutFixed, {
            field,
            value,
...
class TextareaInput extends ValidationBase {
    ...
    view({ attrs }) {
        const { field, value, xform = value } = attrs;
        const { label, id, name = id, title = label, placeholder, required, readonly, disabled, autofocus, autocomplete, minlength, maxlength, tabindex, spellcheck, instant, uiClass = {} } = attrs.field;
        return m(LayoutTop, { field, value, xform, invalid: this.invalid }, m("textarea.w-100.bg-transparent.bn.outline-0.h-100.resize-none", {
            id, name, title,
...
class OmniFileInput {
    ...
    view({ attrs: { field, value } }) {
        ...
        return m("div", {
            ...
            ? [
                // Image preview
                m(".relative.w-100.dt.tc", m("img.img.contain.max-h-img", {
                    title: file.name,
                    src: imgSrc(file.path, file.dataUrl)
                }), !(readonly || disabled) && m(".absolute.top-0.right-0.pa1.pointer.dim", {
                    title: `Remove ${file.name}`,
